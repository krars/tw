...
var WINDOW_LAMP_ON = "#2fc44f",
  ...
  AUTO_ENTER_PLAYER_NAME = "Kolyan454",
...
function addTimer() {
  try {
    ...
    canvasStyle.value =
      "height:" + CANVAS_SIZE + "px;width:" + CANVAS_SIZE + "px;display:block";
    canvasTdStyle.value =
      "position:relative;overflow:visible;min-width:" + CANVAS_SIZE + "px;";
    ...
    secondStyle.value =
      "position:absolute;left:50%;top:50%;transform:translate(-50%, -52%);margin:0;font-size:28px;line-height:1;";
    ...
    windowLampStyle.value =
      "position:absolute;top:6px;left:6px;width:18px;height:18px;border:2px solid #2a2a2a;border-radius:50%;background:" +
      WINDOW_LAMP_OFF +
      ";box-shadow:0 0 3px rgba(0,0,0,0.9);z-index:30;display:block;";
...
function drawDial(ctx) {
  var i, ms, angle, cosA, sinA, isMajor, inner, outer, label;
  ...
  for (label = 0; label < 1000; label += 100) {
    ...
    ctx.fillText(
      String(label),
      CANVAS_CENTER + (CANVAS_RADIUS - 16) * Math.cos(angle),
      CANVAS_CENTER + (CANVAS_RADIUS - 16) * Math.sin(angle),
    );
  }
...
function getCurrentReferenceTimeMs() {
  var timingApi, methods, i, serverNow;
  try {
    if (typeof Timing !== "undefined" && Timing) {
      timingApi = Timing;
      methods = [
        "getCorrectedServerTime",
...
function formatTargetWindowTime(msOfDay) {
  ...
  return (
    pad2(hours) + ":" + pad2(minutes) + ":" + pad2(seconds) + ":" + pad3(millis)
  );
}
...
function getCurrentServerClockMsOfDay() {
  var serverTimeEl = document.getElementById("serverTime"),
    match = serverTimeEl
      ? String(serverTimeEl.textContent || "").match(
          /(\d{1,2}):(\d{1,2}):(\d{1,2})/,
        )
      : null,
...
function getCommandDurationMs() {
  ...
  if (tableBody) {
    ...
    durationText =
      durationRow && durationRow.children && durationRow.children[1]
        ? durationRow.children[1].textContent
        : "";
...
function getPredictedArrivalMsOfDay() {
  return normalizeMsOfDay(
    Math.floor(getCurrentReferenceTimeMs() + getCommandDurationMs()),
  );
}
...
function triggerEnterPress() {
  var submitBtn = $("#troop_confirm_submit")[0],
    formEl =
      submitBtn && submitBtn.form ? submitBtn.form : $("#command-data-form")[0],
    target =
...
function maybeAutoEnterAtHalfWindow(startMsOfDay, endMsOfDay, etaMsOfDay) {
  ...
  passedHalf =
    elapsedFromStartMs >= Math.floor(windowSpanMs / 2) &&
    elapsedFromStartMs <= windowSpanMs + AUTO_ENTER_GRACE_MS;
  if (!passedHalf) {
  ...
  if (triggerEnterPress()) {
    ...
    console.log(
      "[timer.js] auto-enter trigger attempted but submit did not confirm",
    );
  }
}
...
function buildDefaultWindowValues(tableBody) {
  ...
  startMsOfDay = normalizeMsOfDay(
    predictedArrivalMsOfDay + TARGET_WINDOW_DEFAULT_SHIFT_MS,
  );
...
function buildWindowValuesFromCenterMs(centerMsOfDay) {
  var startMsOfDay = normalizeMsOfDay(
      centerMsOfDay - TARGET_WINDOW_CENTER_HALF_SPAN_MS,
    ),
    endMsOfDay = normalizeMsOfDay(
      centerMsOfDay + TARGET_WINDOW_CENTER_HALF_SPAN_MS,
    );
  return {
    startMsOfDay: startMsOfDay,
...
function updateWindowLamp() {
  var lamp = $("#window_lamp")[0],
    ...
    startMsOfDay = parseTargetWindowValue(
      targetWindowStartInput ? targetWindowStartInput.value : "",
    ),
    endMsOfDay = parseTargetWindowValue(
      targetWindowEndInput ? targetWindowEndInput.value : "",
    ),
    isWindowValid,
    isPredictedInWindow,
    isActive,
  ...
  isPredictedInWindow =
    isWindowValid &&
    isTimeInWindow(predictedArrivalMsOfDay, startMsOfDay, endMsOfDay);
  isActive = isPredictedInWindow;
  isPreActive =
    !isActive &&
    isWindowValid &&
    isTimeInPreWindow(predictedArrivalMsOfDay, startMsOfDay);
  if (isWindowValid) {
    maybeAutoEnterAtHalfWindow(
      startMsOfDay,
      endMsOfDay,
      predictedArrivalMsOfDay,
    );
  }
  debugTitle =
    "now:" +
    formatTargetWindowTime(nowMsOfDay) +
    " | eta:" +
    formatTargetWindowTime(predictedArrivalMsOfDay) +
    " | start:" +
    (isNaN(startMsOfDay) ? "NaN" : formatTargetWindowTime(startMsOfDay)) +
    " | end:" +
    (isNaN(endMsOfDay) ? "NaN" : formatTargetWindowTime(endMsOfDay)) +
    " | inEta:" +
    (isPredictedInWindow ? "1" : "0") +
    " | auto:" +
    lastAutoEnterDebug +
    " | state:" +
    (isActive ? "green" : isPreActive ? "yellow" : "off");
  for (i = 0; i < lamps.length; i++) {
    lamps[i].title = debugTitle;
    if (isActive) {
      lamps[i].style.background = WINDOW_LAMP_ON;
      lamps[i].style.boxShadow =
        "0 0 8px " + WINDOW_LAMP_ON + ", 0 0 2px rgba(0,0,0,0.8)";
    } else if (isPreActive) {
      lamps[i].style.background = WINDOW_LAMP_PRE;
      lamps[i].style.boxShadow =
        "0 0 8px " + WINDOW_LAMP_PRE + ", 0 0 2px rgba(0,0,0,0.8)";
    } else {
...
function updateTargetFrameLayout() {
  var viewportWidth, viewportHeight, compact, desktopHeight, compactHeight;
...
function scrollTargetFrameToThirtyPercent() {
  var frameWin, frameDoc, docEl, body, scrollHeight, targetTop;
  try {
...
function createTargetFrame() {
  var frameHead, frameTitle, closeBtn;
...
function openTargetFrame() {
  var href = targetVillageHref,
    ...
    fallbackBody =
      infoTable && infoTable.tBodies && infoTable.tBodies.length
        ? infoTable.tBodies[0]
        : null;
  if (!href) {
