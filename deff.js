...
`; 
        ...
        apply_window_position: function (root) {
            ...
            const position = Guard.clamp_window_position(root, Guard.load_window_position() || {
                left: fallback_left,
                top: 72,
            });
        ...
        bind_window_drag: function (root) {
            const handle = root && root.querySelector('.guard-drag-handle');
            if (!handle) return;
            let dragging = false;
            let start_x = 0;
            let start_y = 0;
            ...
            const on_pointer_move = event => {
                ...
                const position = Guard.clamp_window_position(root, {
            ...
            const on_pointer_up = event => {
                if (!dragging) return;
                dragging = false;
                root.classList.remove('guard-dragging');
            ...
            handle.addEventListener('pointerdown', event => {
                ...
                start_top = rect.top;
                root.classList.add('guard-dragging');
                try {
        ...
        create_gui: function () {
            ...
            root.style.margin = '0';
            root.append(Guard.create_drag_handle());
